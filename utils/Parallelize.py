...
class SplitJoinParallel(object):
    ...
    def __init__(self, Runner, seeds = None, njobs = -1):
        ...
        self.Runner_list = split_run(self.Runner)
    ...
    def split_run(self, Runner):
        ...
        for i in range(Nsplits):
            ...
            New_HaloCatalog = type(HaloCatalog)(ra = catalog['ra'][start:end], dec = catalog['dec'][start:end],
                                                M = catalog['M'][start:end],   z = catalog['z'][start:end], cosmo = cosmo)
            
            New_Runner = type(self.Runner)(New_HaloCatalog, empty_shell, config)
    ...
    def single_run(self, Runner):        
        ...
        return Runner.process(verbose = False)
